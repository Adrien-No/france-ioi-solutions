...
![](pics/solution-geometrique-sans-epsilon_optimisation.png)
...
![](pics/solution-geometrique-AVEC-epsilon_optimisation.png)
...
Soit (x,y) les coordonnées d'un point du tableau t, on note pgcl (pour Plus Grand Carré Libre) la fonction de récurrence (mémoïsée).
    ...
    pgcl(x,y) = 0                                                 si t.(x).(y) == True  
                1 + min (pgcl(x+1,y), pgcl(x,y+1), pgcl(x+1,y+1)) sinon  
```
